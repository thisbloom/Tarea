...
@section('content')
    ...
    <div class="container">
        @foreach($resultadosCompra as $compra)

            <div class="card mt-5 border-5 pt-2 active pb-0 px-3">
                <div class="card-body ">
                    <div class="row">
                        <div class="col-12 ">
                            <h4 class="card-title "><b>@foreach($resultadosCurso as $Curso) @if($compra["id_curso"]==$Curso["id"]) {{$Curso["nombre_curso"]}} @endif @endforeach</b></h4>
                        </div>
                        <div class="col">
                            <h6 class="card-subtitle mb-2 text-muted">
                                <p class="card-text text-muted small "> <img src="https://i.imgur.com/UkfzmBw.png" class="mr-1 " width="19" height="19" id="star"> <span class="vl mr-2 ml-0"></span> Creado por <span class="font-weight-bold"> @foreach($resultadosCurso as $Curso) @if($compra["id_curso"]==$Curso["id"]) @foreach($res as $user) @if($Curso["creador_curso"]==$user["id"]) {{$user["name"]}} @endif @endforeach @endif @endforeach @foreach($res as $user) @if($compra["creador_curso"]==$user["id"]) {{$user["name"]}} @endif @endforeach </span> </p>
                            </h6>
                            <h6>
                                <p class="card-text text-muted small "> <img src="https://i.imgur.com/TfHvSK3.png" class="mr-1 " width="19" height="19" id="star"> <span class="vl mr-2 ml-0"></span> Descripción del curso: @foreach($resultadosCurso as $Curso) @if($compra["id_curso"]==$Curso["id"]) {{$Curso["desc_curso"]}} @endif @endforeach</p>
                            </h6>
                        </div>
                    </div>
                </div>
                <div class="card-footer bg-white px-0 ">
                    <div class="row">
                        <div class=" col-md-auto "> <a class="btn btn-outlined btn-black text-muted bg-transparent" data-wow-delay="0.7s"><img src="https://i.imgur.com/VvTUMvO.png" width="19" height="19"> <small>@foreach($resultadosCurso as $Curso) @if($compra["id_curso"]==$Curso["id"]) {{$Curso["precio_curso"]}} @endif @endforeach</small></a> <i class="mdi mdi-settings-outline"></i> <a href="/DetallesCurso/{{$compra["id_curso"]}}" class=" btn-outlined btn-black text-muted"><img src="https://img.icons8.com/metro/26/000000/link.png" width="17" height="17" id="plus"> <small>ENLACE DEL CURSO</small> </a>  <span class="vl ml-3"></span> </div>
                        <div class="col-md-auto ">
                            <ul class="list-inline">
                                <li class="list-inline-item"> <img src="@foreach($resultadosCurso as $Curso) @if($compra["id_curso"]==$Curso["id"]) {{asset("storage/".$Curso["img_curso"])}} @endif @endforeach" alt="DP" class=" rounded-circle img-fluid " width="35" height="35"> </li>
				...
				s33.181,12.591,38.725,16.498c2.387,1.682,3.461,4.668,2.705,7.488L380.857,346.164z" />
					...
					278.436,375.599 383.003,264.076 364.393,251.618 264.807,364.928 				" />
                        ...
                        <div class="col-md-6 align-self-center">
                            <h1>404</h1>
                            <h2>UH OH! </h2>
                            <p>Parece que no tienes cursos comprados ve a la pestaña de <a href="{{ url('/BuscarCursos') }}">comprar cursos</a> para comprar tu primer curso
                        </div>
                    </div>
                </div>
            </main>
